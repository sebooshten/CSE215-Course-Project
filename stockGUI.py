...
class StockApp:
            ...
            for stock in self.stock_list:
                if stock.symbol == symbol:
                    ...
                    for daily_data in stock.DataList:
                        row = daily_data.date + " " + '${:0,.2f}'.format(daily_data.close) + " " + str(
                            daily_data.volume) + "\n"
    ...
    def importCSV_web_data(self):
        symbol = self.stockList.get(self.stockList.curselection())
        filename = filedialog.askopenfilename(title="Select " + symbol + " File to Import",
